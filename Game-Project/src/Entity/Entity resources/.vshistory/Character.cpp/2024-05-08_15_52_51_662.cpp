...
Characters::Characters() : m_Player(m_Player), m_Friend(m_Friend),
m_Foe(m_Foe), m_World(m_World), m_Globe(m_Globe), playerText(playerText)
{
	std::string Player = m_Loader.m_FullPath + "/mobss.png";
	if (playerText->loadFromFile(Player))
	{
		std::cout << "success";
	}
}
