...
Player::Player(Window& window, InputManager& inputManager) :Character(m_PlayerText),en(en), m_Input(inputManager), m_Window(window), m_DeltaTime(m_DeltaTime)
{
...
//	&& (!m_Check.IsColliding(m_Player, m_Foe) &&
	...
	// Collision
	//sf::Vector2f enemyPosition = m_Enemy.GetEnemy().getPosition();
	//sf::Sprite* enemy = &m_Enemy.GetEnemy();
	sf::Sprite m_Enemy = en.GetEnemy();
	std::cout << m_Enemy.getPosition().x;
